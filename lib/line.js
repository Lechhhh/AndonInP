...
function initial() {
    const now=Date.now();
    return { schema:2, goal:15,netShiftMins:420,taktMins:28,count:0,accDown:0,breakOverlayDisabled:false,isDown:false,isBreak:false,shiftActive:false,shiftStart:null,shiftStopTime:now,cycleStart:now,cycleId:randomUUID(),shiftId:randomUUID(),st:Object.fromEntries(STATIONS.map(id=>[id,{r:false,s:false,reason:null,supportUserId:null}])),logs:[],cycleTimes:[],planner:[],outbox:[],requests:[] };
}
class Line {
    constructor(file,vault,clock=Date.now,history=null) {
        ...
        for(const id of STATIONS)if(!s.st[id]||typeof s.st[id].r!=='boolean'||typeof s.st[id].s!=='boolean')throw new Error('Invalid station state.');
        ...
        s.taktMins=s.netShiftMins/s.goal;
    ...
    snapshot(view,user) {
        const s=this.state,t=this.timing(),base={breakOverlayDisabled:s.breakOverlayDisabled,goal:s.goal,netShiftMins:s.netShiftMins,taktMins:s.taktMins,count:s.count,accDown:s.accDown,shiftActive:s.shiftActive,shiftStart:s.shiftStart,shiftStopTime:s.shiftStopTime,cycleStart:s.cycleStart,cycleId:s.cycleId,shiftId:s.shiftId,...t};
        const ids=STATIONS.includes(view)?[view]:view==='tv-electro'?stationConfig.departments.electro:view==='tv-assembly'?stationConfig.departments.assembly:STATIONS;
    ...
    begin(s,reason) {
        const now=this.clock();s.count=0;s.accDown=0;s.shiftActive=true;s.shiftStart=now;s.shiftStopTime=null;s.cycleStart=now;s.cycleId=randomUUID();s.shiftId=randomUUID();s.cycleTimes=[];s.isDown=false;s.isBreak=false;s.breakStart=null;s.downStart=null;
        for(const id of STATIONS)s.st[id]={r:false,s:false,reason:null,supportUserId:null};
    ...
    change(event,data,user,view) {
        ...
        if(event==='actionOK') {
            if(s.count>=s.goal)throw new PublicError('GOAL_REACHED','Plan został wykonany. Rozpoczęcie nowej zmiany wymaga decyzji brygadzisty.',409);
            if(!s.shiftActive||timing.isBreak)throw new PublicError('LINE_PAUSED','Nie można potwierdzić operacji podczas przerwy lub zatrzymanej zmiany.',409);
            if(data.cycleId!==s.cycleId)throw new PublicError('STALE_CYCLE','Ten cykl został już zakończony. Odśwież dane.',409);
            if(s.st[view].r)throw new PublicError('ALREADY_CONFIRMED','Stanowisko potwierdziło już ten cykl.',409);
            s.st[view].r=true;const seconds=Math.floor(timing.cycleNetMs/1000);
            s.cycleTimes.unshift({at:now,employeeId:user.id,s:stationLabel(view),t:s.count+1,m:Math.floor(seconds/60)+'m '+seconds%60+'s',netSeconds:seconds,grossSeconds:Math.floor(timing.cycleGrossMs/1000),ts:new Date(now).toLocaleTimeString('pl-PL')});s.cycleTimes=s.cycleTimes.slice(0,200);
            const cycleStations=stationConfig.department(view)==='assembly'?stationConfig.departments.assembly:stationConfig.departments.electro;if(cycleStations.every(id=>s.st[id].r)){s.count++;s.accDown+=timing.currentDown;s.cycleStart=now;s.cycleId=randomUUID();s.isDown=false;for(const id of cycleStations)s.st[id].r=false;log(s,'Cykl zamknięty','Wykonano: '+s.count+'/'+s.goal);}
            if(s.count>=s.goal){s.shiftActive=false;s.shiftStopTime=now;s.isBreak=false;s.breakStart=null;s.downStart=null;log(s,'Plan wykonany — zmiana zakończona','Wykonano: '+s.count+'/'+s.goal);}
            ...
            if(s.shiftActive)throw new PublicError('ACTIVE_SHIFT','Zatrzymaj zmianę przed zmianą celu i taktu.',409);
            s.goal=data.goal;s.netShiftMins=data.time;s.taktMins=data.time/data.goal;log(s,'Zmieniono parametry linii','Cel: '+s.goal+' szt.');
            ...
            if(s.shiftActive)throw new PublicError('ACTIVE_SHIFT','Zmiana już trwa. Zatrzymaj ją przed rozpoczęciem nowej.',409);
            this.begin(s,'Zmiana rozpoczęta ręcznie');
        } else if(event==='shiftStop') {
            if(!s.shiftActive)throw new PublicError('STOPPED_SHIFT','Zmiana jest już zatrzymana.',409);
            s.shiftActive=false;s.shiftStopTime=now;log(s,'Zmiana zatrzymana','Wykonano: '+s.count+'/'+s.goal);
