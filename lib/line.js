...
class Line {
    ...
    snapshot(view,user) {
        ...
        const ids=STATIONS.includes(view)?[view]:STATIONS;
        ...
        base.logs=view==='panel'||view==='tv'?s.logs.map(({t,m,d,type})=>({t,m,d,type})):[];
        base.cycleTimes=view==='panel'||view==='tv'?s.cycleTimes.map(({s,t,m,ts,netSeconds,grossSeconds})=>({s,t,m,ts,netSeconds,grossSeconds})):[];
    ...
    change(event,data,user,view) {
        ...
        if(event==='actionOK') {
            ...
            if(STATIONS.every(id=>s.st[id].r)){s.count++;s.accDown+=timing.currentDown;s.cycleStart=now;s.cycleId=randomUUID();s.isDown=false;for(const id of STATIONS)s.st[id].r=false;log(s,'Cykl zamknięty','Wykonano: '+s.count+'/'+s.goal);}
