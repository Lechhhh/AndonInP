...
class Line {
    constructor(file,vault,clock=Date.now,history=null) {
        ...
        s.taktMins=s.netShiftMins/s.goal;s.assemblyGoal=Number.isInteger(s.assemblyGoal)?s.assemblyGoal:s.goal;s.assemblyNetShiftMins=Number.isFinite(s.assemblyNetShiftMins)?s.assemblyNetShiftMins:s.netShiftMins;s.assemblyTaktMins=s.assemblyNetShiftMins/s.assemblyGoal;s.assemblyCount=Number.isInteger(s.assemblyCount)?s.assemblyCount:0;s.assemblyCycleStart=Number.isFinite(s.assemblyCycleStart)?s.assemblyCycleStart:s.cycleStart;s.assemblyCycleId=typeof s.assemblyCycleId==='string'?s.assemblyCycleId:randomUUID();
    ...
    snapshot(view,user) {
        const s=this.state,isAssembly=view==='tv-assembly'||stationConfig.department(view)==='assembly',lane=isAssembly?{goal:s.assemblyGoal,netShiftMins:s.assemblyNetShiftMins,taktMins:s.assemblyTaktMins,count:s.assemblyCount,cycleStart:s.assemblyCycleStart,cycleId:s.assemblyCycleId}:{goal:s.goal,netShiftMins:s.netShiftMins,taktMins:s.taktMins,count:s.count,cycleStart:s.cycleStart,cycleId:s.cycleId},timingState=isAssembly?{...s,...lane}:s,t=this.timing(timingState),base={breakOverlayDisabled:s.breakOverlayDisabled,...lane,assemblyGoal:s.assemblyGoal,assemblyNetShiftMins:s.assemblyNetShiftMins,assemblyTaktMins:s.assemblyTaktMins,assemblyCount:s.assemblyCount,accDown:s.accDown,shiftActive:s.shiftActive,shiftStart:s.shiftStart,shiftStopTime:s.shiftStopTime,shiftId:s.shiftId,...t};
    ...
    begin(s,reason) {
        const now=this.clock();s.count=0;s.assemblyCount=0;s.accDown=0;s.shiftActive=true;s.shiftStart=now;s.shiftStopTime=null;s.cycleStart=now;s.cycleId=randomUUID();s.assemblyCycleStart=now;s.assemblyCycleId=randomUUID();s.shiftId=randomUUID();s.cycleTimes=[];s.isDown=false;s.isBreak=false;s.breakStart=null;s.downStart=null;
    ...
    change(event,data,user,view) {
        ...
        if(event==='actionOK') {
            ...
            if(!s.shiftActive||laneTiming.isBreak)throw new PublicError('LINE_PAUSED','Nie można potwierdzić operacji podczas przerwy lub zatrzymanej zmiany.',409);
            ...
            if(s.count>=s.goal&&s.assemblyCount>=s.assemblyGoal){s.shiftActive=false;s.shiftStopTime=now;s.isBreak=false;s.breakStart=null;s.downStart=null;log(s,'Plany wykonane - zmiana zakończona','Elektromontaż: '+s.count+'/'+s.goal+' · Montaż: '+s.assemblyCount+'/'+s.assemblyGoal);}
