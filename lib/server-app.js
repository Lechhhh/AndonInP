...
async function createApplication(config) {
    ...
    app.post('/api/login',async(req,res)=>{
        ...
        limiter.take('login-ip:'+ip(req),15,10000);
        const data=validate('login',req.body);
        limiter.take('login-code:'+vault.lookup(data.code),10,10000);
    ...
    app.post('/api/co-login',async(req,res)=>{
        const session=sessions.from(req);sessions.csrf(session,req.headers['x-csrf-token']);
        limiter.take('co-login-ip:'+ip(req),15,10000);
        const data=validate('coLogin',req.body);
        const user=await accounts.authenticate(data.code);
        if(!user)throw new PublicError('BAD_LOGIN','Nieprawidłowy numer lub konto jest nieaktywne.',401);
        res.json({ok:true,userName:user.name,employeeId:user.id});
    });
    ...
    function canView(user,view){return STATIONS.includes(view)?user.allowedStations.includes(view):['owner','manager'].includes(user.role)&&['tv','tv-electro','tv-assembly','panel'].includes(view);}
