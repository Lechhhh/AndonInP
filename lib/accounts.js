...
const STATIONS = [...stationConfig.departments.electro];
const ALL_STATIONS = stationConfig.ids();
...
class Accounts {
    ...
    static async open(file, vault) {
        ...
        for (const user of store.state.users) {
            if (!user.allowedStations) { user.allowedStations = [...ALL_STATIONS]; changed = true; }
            if (!Array.isArray(user.allowedStations) || user.allowedStations.some(s => !ALL_STATIONS.includes(s))) throw new Error('Invalid station assignments.');
    ...
    list(query = {}) {
        ...
        return { version: this.state.version, users: matched.slice((page - 1) * limit, page * limit).map(visible), total: matched.length, page, limit,
            audit: this.state.audit.slice(-100).reverse().map(e => ({ id: e.id, at: e.at, actorName: e.actorName, action: e.action, userId: e.userId, after: e.after ? visible({ ...e.after, allowedStations: e.after.allowedStations || ALL_STATIONS }) : null })) };
    }
...
module.exports = { Accounts, ROLES, STATIONS, ALL_STATIONS, validCode, visible };
