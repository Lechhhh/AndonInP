...
const { ALL_STATIONS } = require('./accounts');
const STATIONS=ALL_STATIONS;
const text = max => z.string().trim().min(1).max(max).refine(s => !/[\u0000-\u001f\u007f]/.test(s));
const id = z.uuid();
const person = z.object({ number: z.string().regex(/^\d{8}$/), name: text(100), role: z.enum(['employee', 'manager', 'owner']), active: z.boolean(), allowedStations: z.array(z.enum(STATIONS)).max(STATIONS.length).refine(v => new Set(v).size === v.length), version: z.number().int().positive() }).strict();
...
const schemas = {historyQuery,historyExport:historyQuery,
    ...
    coLogin: z.object({ code: z.string().regex(/^\d{8}$/) }).strict(),
    view: z.enum([...STATIONS, 'panel', 'tv', 'tv-electro', 'tv-assembly']),
    ...
    adminSettings: z.object({ goal: z.number().int().min(1).max(10000), time: z.number().int().min(1).max(1440), department: z.enum(['electro','assembly']).default('electro'), requestId: id }).strict(),
