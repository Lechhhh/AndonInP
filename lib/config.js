...
function configFrom(env = process.env, root = path.resolve(__dirname, '..')) {
    ...
    const cfg = {
        ...
        renderProxy: env.ANDON_TRUST_PROXY === 'render',
    ...
    if (production && !cfg.origins.length) throw new Error('Produkcja wymaga ANDON_ALLOWED_ORIGINS: pełny adres HTTPS aplikacji bez końcowego ukośnika.');
    if (production && !cfg.directTls && !cfg.proxy) throw new Error('Produkcja wymaga TLS lub ANDON_TRUST_PROXY dla HTTPS przez proxy. Publiczny test na Render obsługuje ANDON_TRUST_PROXY=render.');
    ...
    if (cfg.renderProxy) {
        if (env.RENDER !== 'true' || !production || !cfg.publicTestMode || cfg.directTls) throw new Error('ANDON_TRUST_PROXY=render wymaga środowiska Render, NODE_ENV=production i ANDON_PUBLIC_TEST_MODE=true, bez bezpośredniego TLS.');
        // Trust only the immediate platform proxy. Never trust the whole XFF chain.
        // Additional proxies can share a rate-limit bucket; this mode cannot use an IP allowlist.
        cfg.proxy = 1;
    }
