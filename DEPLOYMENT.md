...
## 10. Konfiguracja bezpieczeństwa i powiadomień Discord

Webhook nie jest już zapisany w plikach dostępnych dla przeglądarki. Przed uruchomieniem ustaw na stacji-serwerze zmienną środowiskową `DISCORD_WEBHOOK_URL` z nowym adresem webhooka. Poprzedni adres należy unieważnić i wygenerować ponownie, ponieważ był wcześniej widoczny w kodzie klienta.

Opcjonalnie ustaw także `ANDON_ADMIN_PASSWORD` i `ANDON_OPERATOR_PASSWORD`. Hasła są weryfikowane przez serwer, a nie przez kod HTML. Jeżeli zmienne nie zostaną ustawione, zachowane są dotychczasowe wartości zgodnościowe.

Przykład PowerShell przed uruchomieniem:

```powershell
$env:DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/NOWY_ADRES"
$env:ANDON_ADMIN_PASSWORD="silne_haslo"
$env:ANDON_OPERATOR_PASSWORD="haslo_operatora"
.ndon.exe
```

Dla autostartu ustaw te zmienne trwale w środowisku użytkownika lub uruchamiaj aplikację przez zabezpieczony skrypt konfiguracyjny niedostępny dla zwykłych użytkowników.
